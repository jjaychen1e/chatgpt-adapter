...
build-linux:
	${ENV} GOARCH=amd64 GOOS=linux go build -gcflags=all="-N -l" -toolexec iocgo -ldflags="-s -w" -o bin/linux/${TARGET_EXEC} -trimpath main.go

build-linux-arm64:
	${ENV} GOARCH=arm64 GOOS=linux go build -gcflags=all="-N -l" -toolexec iocgo -ldflags="-s -w" -o bin/linux/${TARGET_EXEC}-arm64 -trimpath main.go
